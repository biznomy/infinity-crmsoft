...
# Extract and return all emails found in a block of text
def get_all_contacts(page_contents):
	contacts = re.findall(r"+\d{2}\s?0?\d{10}", page_contents, re.I)
...
def crawl_site(seed, domain, max_pages=30):
	...
	while len(to_crawl) and (len(crawled) < max_pages):
		...
		if url.endswith(('.mp3', '.jpg', 'jpeg', 'png')) : print url
		else: crawled.add(url)
...
if __name__ == "__main__":
	def parse_options():
		...
		parser.add_argument("-m", "--maxpages", action="store", type=int,
		                    default=30, help="maximum number of pages to crawl")
		parser.add_argument("-v", "--verbose", action="store_true",
		                    help="increase output verbosity", default=False)
		parser.add_argument("--dataId", action="store",
		                    help="domain from dataTable Id")
	...
	def main():
		...
		result  = list(emails_found)
		
		noU = []
		for data in emails_found:
			A = str(data)
			noU.append(A)
