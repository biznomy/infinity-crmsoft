...
$(document).on("click", "#btn", function() {
	...
	console.log(obj);
	if (validationforms.validateformPerson()) {
		// alert("hiii this is error");
		alertFunction("Please Fill Compulsory Field", 'danger', '#person');
					...
					//	url : 'http://localhost:9090/crmsoft/contentdata?',
						...
						data : function(params) {
							return {
								jsq : '{"query":{"edContentData":{"filter":{"title":{"like":"'
										+ params.term
										+ '"},"type":{"equals":"company"}}}},"page":{"start":1,"max":10}}', // search term
						...
						processResults : function(data) {
							...
							for ( var mydata in data.results) {
								...
								//obj['text'] = data.results[mydata].title+" id="+data.results[mydata].id;
								arrayobj.push(obj);
							...
							$('#selector1').on(
									"select2:select",
									function(e) {
										var selectedVal = $('#selector1').find(":selected").val();
										$("#cId").val(selectedVal);
									})
							return {
								results : arrayobj
